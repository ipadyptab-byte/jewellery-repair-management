          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 12, alignItems: 'flex-end' }}>
            <div className="field" style={{ flex: 1, marginBottom: 0 }}>
              ...
              <select value={koDoc} onChange={e => { setKoDoc(e.target.value); setKoLoaded(false) }}>
                ...
                {koEditing 
                  ? records.filter(r => r.status === 'with_karagir').map(r => <option key={r.docNum} value={r.docNum || ''}>{r.docNum} — {r.name} ({r.jewellery}) [{r.current_location}]</option>)
                  : records.filter(r => r.status === 'received' || r.status === 'with_karagir').map(r => <option key={r.docNum} value={r.docNum || ''}>{r.docNum} — {r.name} ({r.jewellery}) [{r.current_location}]</option>)
                }
              </select>
            </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 12, alignItems: 'flex-end' }}>
            <div className="field" style={{ flex: 1, marginBottom: 0 }}>
              ...
              <select value={kiDoc} onChange={e => { setKiDoc(e.target.value); setKiLoaded(false); setFinalRec(null) }}>
                ...
                {kiEditing 
                  ? records.filter(r => r.status === 'ready').map(r => <option key={r.docNum} value={r.docNum || ''}>{r.docNum} — {r.name} ({r.jewellery}) [{r.current_location}]</option>)
                  : records.filter(r => r.status === 'with_karagir').map(r => <option key={r.docNum} value={r.docNum || ''}>{r.docNum} — {r.name} ({r.jewellery}) [{r.current_location}]</option>)
                }
              </select>
            </div>
