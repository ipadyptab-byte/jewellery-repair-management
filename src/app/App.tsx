          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 12, alignItems: 'flex-end' }}>
            <div className="field" style={{ flex: 1, marginBottom: 0 }}>
              ...
              <select value={koDoc} onChange={e => { setKoDoc(e.target.value); setKoLoaded(false) }}>
                ...
                {koEditing 
                  ? records.filter(r => r.status === 'with_karagir' && (r.current_location === cfgLocation || r.location === cfgLocation)).map(r => <option key={r.docNum} value={r.docNum || ''}>{r.docNum} — {r.name} ({r.jewellery}) {r.location !== cfgLocation ? '(From '+r.location+')' : ''}</option>)
                  : records.filter(r => (r.status === 'received' || r.status === 'with_karagir') && (r.current_location === cfgLocation || r.location === cfgLocation)).map(r => <option key={r.docNum} value={r.docNum || ''}>{r.docNum} — {r.name} ({r.jewellery}) {r.location !== cfgLocation ? '(From '+r.location+')' : ''}</option>)
                }
              </select>
            </div>
              ...
              <div className="meta-grid">
                ...
                <div className="meta-item"><div className="meta-label">Items</div>
                  <div className="meta-val" style={{textAlign:'left'}}>
                    ...
                    {(koRecord.repair_items || []).map((item: {metal?: string; jewellery?: string}, i: number) => <div key={i}>• {item.metal} {item.jewellery}</div>)}
                  </div>
                </div>
              ...
              <div className="meta-grid">
                ...
                <div className="meta-item"><div className="meta-label">Items</div>
                  <div className="meta-val" style={{textAlign:'left'}}>
                    ...
                    {(kiRecord.repair_items || []).map((item: {metal?: string; jewellery?: string}, i: number) => <div key={i}>• {item.metal} {item.jewellery}</div>)}
                  </div>
                </div>
