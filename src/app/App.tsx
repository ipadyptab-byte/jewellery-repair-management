          ...
          {kiLoaded && kiRecord && (kiEditing || !finalRec) && (
            <>
              ...
              <div className="btn-row">
                ...
                <button className="btn btn-primary" onClick={async () => { const rec = await saveKI(); if (rec && !kiEditing) { if (trReady && cfgLocation === 'satara') { const received = rec.receivedDate || rec.received_date; const delivery = rec.deliveryDate || rec.delivery_date; const expDays = (received && delivery) ? Math.max(1, Math.ceil((new Date(delivery).getTime() - new Date(received).getTime()) / (1000 * 60 * 60 * 24))) : cfgExpiry; sendWhatsApp(rec, 'final', expDays).catch(console.error); } setSavedRec(rec); setPrintRec(null); } }}>{kiEditing ? '💾 Update' : <><IcPdf />Confirm & Print Thermal Invoice</>}</button>
              </div>
            </>
          )}
